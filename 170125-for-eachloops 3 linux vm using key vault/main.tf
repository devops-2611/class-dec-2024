...
data "azurerm_key_vault_secret" "welcome-user" {
  for_each = var.var-vm
  name         = each.value.uname-secret
  key_vault_id = data.azurerm_key_vault.welcome-kv[each.key].id
}

data "azurerm_key_vault_secret" "welcome-pass" {
  for_each = var.var-vm
  name         = each.value.pass-secret
  key_vault_id = data.azurerm_key_vault.welcome-kv[each.key].id
}
...
resource "azurerm_linux_virtual_machine" "vm-blcok" {
  ...
  admin_username                  = data.azurerm_key_vault_secret.welcome-user[each.key].value 
  admin_password                  = data.azurerm_key_vault_secret.welcome-pass[each.key].value
